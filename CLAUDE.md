# Australian Law MCP Server — Developer Guide
...
Australian Law MCP server providing Australian federal legislation search via Model Context Protocol. Strategy A deployment (Vercel, bundled SQLite DB).
...
1. `scripts/ingest.ts` → fetches from legislation.gov.au → JSON seed files in `data/seed/`
...
- **Federal Register of Legislation** (legislation.gov.au) — Australian Government, Office of Parliamentary Counsel
- **License:** Creative Commons Attribution 4.0 International (CC BY 4.0)
- **Language:** English (en) — authoritative
- **Coverage:** Key Commonwealth (federal) Acts covering privacy, cybersecurity, critical infrastructure, corporations, consumer, telecommunications, and criminal law

## Deployment

- Vercel Strategy A: DB bundled in `data/database.db`, included via `vercel.json` includeFiles
- npm package: `@ansvar/australian-law-mcp` with bin entry for stdio
