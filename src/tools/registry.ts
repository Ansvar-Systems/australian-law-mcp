/**
 * Tool registry for Australian Law MCP Server.
...
import { getAustralianImplementations, type GetAustralianImplementationsInput } from './get-australian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Federal Register of Legislation (Australian Government, Office of Parliamentary Counsel). ' +
    ...
    'Call this FIRST when you need to understand what Australian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Australian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Australian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Australian legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Australian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Australia?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Australian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Australian statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that an Australian statute references or aligns with. ' +
      'Australia is not an EU member but Australian laws may reference EU directives/regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Australian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_australian_implementations',
    description:
      'Find all Australian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Australian statutes. ' +
      'Note: Australia does not transpose EU law but may reference or align with EU frameworks voluntarily.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Australian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_australian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Australian legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Australian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Australian statute or provision. ' +
      ...
      'Note: Australia is not bound by EU law; this checks voluntary alignment and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Australian statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_australian_implementations':
          result = await getAustralianImplementations(db, args as unknown as GetAustralianImplementationsInput);
          break;
