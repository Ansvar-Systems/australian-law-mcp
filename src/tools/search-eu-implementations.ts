/**
 * search_eu_implementations — Search EU directives/regulations referenced by Australian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_australian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  australian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as australian_statute_count
  ...
  if (input.has_australian_implementation) {
    sql += ' HAVING australian_statute_count > 0';
  }
