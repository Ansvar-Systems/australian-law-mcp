#!/usr/bin/env tsx
/**
 * Australian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.legislation.gov.au';
const PORTAL_NAME = 'Federal Register of Legislation';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/australian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Australian Law MCP — Data Freshness Check');
