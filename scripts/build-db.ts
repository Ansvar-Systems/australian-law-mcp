#!/usr/bin/env tsx
/**
 * Database builder for Australian Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Australian Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Australian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'AU');
    insertMeta.run('source', 'legislation.gov.au');
